...
-- STEP 6: Secure gallery tables
ALTER TABLE galeria ENABLE ROW LEVEL SECURITY;
ALTER TABLE imagens_galeria ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view their own galleries" ON galeria;
DROP POLICY IF EXISTS "Users can insert their own galleries" ON galeria;
DROP POLICY IF EXISTS "Users can update their own galleries" ON galeria;
DROP POLICY IF EXISTS "Users can delete their own galleries" ON galeria;
...
-- STEP 7: Update security definer functions to prevent injection
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE WARNING 'Campo atualizado_em não existe na tabela %', TG_TABLE_NAME;
        RETURN NEW;
    END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING 'Erro no trigger handle_updated_at: %', SQLERRM;
        RETURN NEW;
END;
$$;