...
-- 1. POLÍTICAS RLS PARA STORAGE.OBJECTS
-- Adicionar políticas RLS para controlar acesso a arquivos

-- Política para visualização de arquivos (apenas proprietários)
CREATE POLICY "Users can view their own files" 
ON storage.objects 
FOR SELECT 
USING (auth.uid()::text = (storage.foldername(name))[1]);

-- Política para upload de arquivos (apenas proprietários)
CREATE POLICY "Users can upload their own files" 
ON storage.objects 
FOR INSERT 
WITH CHECK (auth.uid()::text = (storage.foldername(name))[1]);

-- Política para atualização de arquivos (apenas proprietários)
...
-- Política para exclusão de arquivos (apenas proprietários)
