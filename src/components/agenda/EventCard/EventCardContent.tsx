...
import { formatPhone, generateWhatsAppUrl, formatCurrency } from './utils';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-base">
        <div>
          ...
          <div className="flex items-center text-sm sm:text-base">
            ...
            <a 
              href={generateWhatsAppUrl(event.phone)}
              target="_blank" 
        ...
        <div>
          <p><strong>Valor Total:</strong> {formatCurrency(event.totalValue)}</p>
          <p><strong>Entrada:</strong> {formatCurrency(event.downPayment)}</p>
          <p><strong>Restante:</strong> {formatCurrency(event.remainingValue)}</p>
        </div>
      </div>
      {event.notes && (
        <div className="mt-4 text-base">
          <p><strong>Observações:</strong> {event.notes}</p>
        </div>
      )}
    </CardContent>
  );
