...
const Survey = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-gray-100 flex items-center justify-center p-6 py-12">
      <Card className={`w-full max-w-2xl glass shadow-lg border-0 overflow-hidden animate-${animation}`}>
        <CardContent className="p-8">
          ...
          {currentQuestionObj.type === 'radio' && currentQuestionObj.options && (
            ...
            >
              {currentQuestionObj.options.map((option) => (
                <div key={option} className="flex items-center space-x-2 rounded-lg border p-4 hover:bg-gray-50">
                ...
                <div key={option} className="flex items-center space-x-2 rounded-lg border p-4 hover:bg-gray-50">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) handleOptionChange(option);
                      else handleOptionChange(option);
                    }}
                    required
                  />
                  <Label htmlFor={option} className="flex-grow cursor-pointer">{option}</Label>
          ...
          {showFollowUp && currentQuestionObj.followUp && (
                    ...
                    <Input
                      ...
                      onBlur={(e) => {
                        ...
                        if (e.target.value) {
                          ...
                          // We store the raw number but display the formatted version
                          handleFollowUpChange(field.label, rawValue);
